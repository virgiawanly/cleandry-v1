...
const updateTotalPrice = function() {
    setTimeout(() => {
        let prices = $('.int-subtotal').map((i, el) => {
...
const updateTotalPayment = function() {
    setTimeout(() => {
        ...
        let totalDiscount = discountType == 'percent' ? (discount / 100) * totalPrice : discount;
        let totalTax = (tax / 100) * totalPrice;
        let totalPayment = totalPrice - totalDiscount + totalTax + additionalCost;
...
$('#open-member-info-modal-button').on('click', async function() {
    ...
    try {
        ...
        modal.find('.o-member-gender').text(res.member.gender == 'M' ? 'Laki-laki' : 'Perempuan');
...
$("#services-table").on("click", ".select-item-button", function() {
    ...
    if (arrItemsId.some((id) => serviceId == id)) {
        let tr = $(`input[name="service_id[]"][value="${serviceId}"]`).closest("tr");
        ...
        let servicePrice =
            `<span>${formatter.format(row.find('td').eq(4).text())}</span><span class="int-price d-none">${row.find('td').eq(4).text()}</span>`;
        let inputQty = `<input type="number" class="form-control" name="qty[]" value="1" min="1">`;
        let subtotal =
            `<span class="o-subtotal">${formatter.format(row.find('td').eq(4).text())}</span><span class="int-subtotal d-none">${row.find('td').eq(4).text()}</span>`;
        let actions =
            `<button type="button" class="button-hapus-barang btn btn-sm btn-danger remove-item-button" title="Hapus Barang"><i class="fas fa-trash"></i></button>`;
        itemsTable.row
            .add([
                rowNumber,
                serviceName,
